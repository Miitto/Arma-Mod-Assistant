...
void cNewScriptFileDialog::createFile(wxCommandEvent& event)
{
	...
	int initType = m_initTypeRadio->GetSelection();
	if (!submod.IsEmpty()) submod = submod.Append("\\");
	if (!name.IsEmpty() && !(name.Contains('\\') || name.Contains('/'))) {
		BaseOptions settings = cSettings::getBaseSettings();
		if (!cFileManager::fileExists(settings.path + submod + "functions\\" + category + "\\" + name + ".sqf")) {
			std::ofstream scriptFile(std::string(settings.path + submod + "functions\\" + category + "\\fn_" + name + ".sqf"));
			scriptFile << "//" << initType << " Do not remove, this is a record of the init type!" << std::endl;
			scriptFile << "// File Created by Miitto's Arma Mod Assistant" << std::endl;
			scriptFile << "#include \"..\\script_component.hpp\"" << std::endl;
			scriptFile.close();
		}
		cFileManager::createCfgFunctions(settings.path.ToStdString(), submod.RemoveLast().ToStdString()); // Recreate CfgFunctions with new or updated File
...
void cNewScriptFileDialog::createCategory(wxCommandEvent& event)
{
	BaseOptions settings = cSettings::getBaseSettings();
	wxString submod = m_submodLB->GetStringSelection();
	...
	if (categoryDialog.ShowModal() == wxID_OK) {
		...
		cFileManager::scanFolders(settings.path + submod.Append("\\") + "functions", categoryList);
		m_catLB->Clear();
...
void cNewScriptFileDialog::newSubmod(wxCommandEvent& event) {
...
void cNewScriptFileDialog::initDialog(wxInitDialogEvent& event) {
	BaseOptions settings = cSettings::getBaseSettings();
...
void cNewScriptFileDialog::submodSelected(wxCommandEvent& event) {
	BaseOptions settings = cSettings::getBaseSettings();
	if (settings.type == "simple") {
		event.Skip();
		return;
	...
	if (!catList.empty()) m_catLB->SetSelection(0);
	event.Skip();
}